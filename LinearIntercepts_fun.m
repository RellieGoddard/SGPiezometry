...
function [Mean_Lengths_X,Mean_Lengths_Y, lengths_x, lengths_y] = LinearIntercepts_fun(ebsd,nx,ny,cutoff,phase,crystal,plotfig,dev)
...
if plotfig == 1
    ...
    sgtitle(['NIntercepts: ', num2str(nx), ' Cutoff Angle: ', num2str(cutoff)])
end
...
for i = 1:nx
    ...
    if dev == 0
        ...
        lineebsd = ebsd(findByYLocation(ebsd,[y_line(1)]));
    end
...
for i = 1:ny
    ...
    if dev == 0
        ...
        lineebsd = ebsd(findByXLocation(ebsd,x_line(1)));
    end
    if isempty(lineebsd(phase)) == 0
        ind = find(lineebsd('indexed').phase == lineebsd(phase).phase(1)); %indices of primary phase
        % preallocate orientaion vector
